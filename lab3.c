               ...
               queue * settelment_queue_current)
{
    ...
    while (floor_queue_current->free_places <= 0
    ...
    if (floor_queue_current->next == NULL) {
        ...
        strcpy(floor_queue_current->list[(floor_queue_current->places) -
                                         (floor_queue_current->
                                          free_places)], str1);
        floor_queue_current->free_places -= 1;
        free(str1);
                    ...
                    queue * settelment_queue_first)
{
    ...
    for (j = 0;
         j <
         (floor_queue_first->places - floor_queue_first->free_places); j++)
